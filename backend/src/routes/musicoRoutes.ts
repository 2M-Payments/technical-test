...
import { MusicoSchema } from "../validations/musicoSchema";

const router = Router();
const musicoController = container.resolve(MusicoController);

router.post("/musicos", validateMiddleware(MusicoSchema), asyncHandler(musicoController.criar.bind(musicoController)));
